...
- `check`：未取得、次いで確認日時の古い順に最大20資料を取得します。SHA-256で重複・応答の変化を記録し、HTMLのスクリプトやナビゲーションを除いた本文、またはPDFから抽出した本文を非公開SQLiteへ保存します。PDF解析はCPU・メモリ・処理時間を制限した別プロセスへ隔離します。旧版で本文が空のまま保存されたPDFは、HTTP 304のまま固定されないよう一度だけ無条件で再取得して根拠を埋め直します。公開JSONには本文を含めず、形式・サイズ・抽出文字数だけを出します。画像だけのPDF、暗号化PDF、破損PDF、制限超過は根拠なしで成功扱いにせず、安全な`invalid-pdf`として記録して再試行します。エラーでは最後に取得できたハッシュを保持し、再試行時刻を指数バックオフで延ばします。
