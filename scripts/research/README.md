...
本文取得キューは最終ポーリング時刻と直近バッチの処理時間・確認件数・エラー件数・304件数・待機件数を運営画面へ公開します。URL、本文、HTTPバリデーターはこの集計へ含めません。キュー内部で予期しない例外が発生しても公式一覧の巡回ループは継続し、限定エラーコードと連続失敗数を表示して次回の本文ポーリングで自動再試行します。発生・復旧は永続障害台帳へ記録します。
