...
公式経路、本文取得、監視停止、バックアップの異常は永続インシデント台帳へ記録します。同じ異常を巡回ごとに新規作成せず、継続回数と最終確認時刻を更新し、復旧時は同じ記録を解決済みにします。発生・復旧の通知候補もSQLiteの保留キューへ保存するため再起動で失いません。ただし外部送信処理は未実装かつ無効で、メール、Slack、SNS等への通知は行いません。通知先・認証情報・費用・送信頻度を承認してから別工程で接続します。
