...
`lib/research/providers.json` をPythonの取得処理とWeb表示で共用しています。現在20銘柄。会社名、分野、公式一覧またはRSSのURL、取得形式、許可ホスト、発表ページのパス規則を設定します。銘柄追加には設定と実際の取得確認が必要で、未検証の銘柄を取得成功に数えません。
